...
var reportsFile = __dirname + "/debug/reports.log"
...
dataServer.prototype = {
  serve: function( path ) {
    ...
    this.app.post( '/sms', function( req, res ) {
      ...
      var logMsg = "" + new Date() + "\n";
      logMsg    += "FROM    : " + req.body.From + "\n";
      logMsg    += "RAW     : " + req.body.Body + "\n";
      logMsg    += util.inspect( report ) + "\n\n";
      fs.appendFile( reportsFile, logMsg, function (err) {
        if (err) throw err;
      });
      res.end()
    })
  ...
  parseReport: function( reportValue ) {
    ...
    {
      if ( reportValue.length != 103 ) {
        console.log( "Poorly formed report.");
        return;
      }
      var report = {};
      report.batteryVoltage = reportValue.readUInt16BE(1);
      report.sensorType = reportValue[3];
      report.date = new Date( reportValue[4],
                           reportValue[5],
                           reportValue[6], 0, 0, 0, 0 );
      var bucketString = reportValue.slice(7);
      report.buckets = [];
      for (var i=0; i<24; ++i)
      {
        report.buckets[i] = bucketString.readUInt32BE(i*4);
      }
