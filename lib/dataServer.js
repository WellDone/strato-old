...
dataServer.prototype = {
  serve: function( path ) {
    ...
    this.app.post( '/sms', function( req, res ) {
      ...
      var meta = {
        ...
        from: req.body.From,
        raw: req.body.Body,
        type: (report.momoReportVersion == 0)?"registration":"report",
        report: report
      };
      reports.push( meta );
  ...
  parseReport: function( reportValue ) {
    ...
    var report = {};
    report.momoReportVersion = reportValue[0];
    if ( report.momoReportVersion == 0 ) {
      // Registration
      report.errorCount = reportValue[1];
      return report;
    }
    else if ( report.momoReportVersion == 1 ) //TODO: Make extensible
    {
