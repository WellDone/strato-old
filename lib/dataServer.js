...
dataServer.prototype = {
  ...
  listen: function( path ) {
    ...
    function processSMSPost( req, res ) {
      ...
      var timestamp = req.body.sent_timestamp? new Date(req.body.sent_timestamp) : null;

      console.log( "from: " + from );
      console.log( "body: " + body );
      console.log( "timestamp: " + req.body.sent_timestamp?req.body.sent_timestamp:new Date() );
  ...
  getReports: function(res, siteID) {
    var reports = {};
    this.db.query( "SELECT aggregate_reports.timestamp, aggregate_reports.eventcount, aggregate_reports.monitorid FROM aggregate_reports INNER JOIN monitors ON monitors.id = aggregate_reports.monitorid WHERE monitors.siteid = " + siteID )
      .on( 'row', function(row) {
