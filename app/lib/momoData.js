...
momoData.prototype = {
  ...
  storeReport: function( report, successCallback ) {
    var monitorID;
    var self = this;
          ...
          .on( 'end', function() {
            if ( monitorID || monitorID === 0 ) {
              self.runStoreReportQuery( monitorID, report )
                .on( 'end', function(){
                  console.log("DONE!");
                  successCallback( { monitor: monitorID,
                                     report: report } );
                });
            } else {
  ...
  processSMS: function( from, body, timestamp, serverEmitter ) {
    if ( !timestamp ) {
      timestamp = new Date();
    }
    ...
    if ( report.isRegistration ) {
      ...
      this.storeReport( report, function( obj ) {
        serverEmitter.emit( 'monitor/' + obj.monitor + '/newReport', { date: obj.report.received, monitor: obj.monitor, eventCount: obj.report.eventCount } );
      } );
      return true;
    }
